...
class Round:
    ...
    def make_games_df(self, mode='exhaustive'): 
        ...
        if mode=='exhaustive': 
            ...
            games = [Game(ident=i, black=competitors[k], white=competitors[k+1]) for k in range(0,n//2, 2)]
        
        games_dict = {g.ident: [g.black, g.white, g.winner] for g in games}
        games_df = pd.DataFrame.from_dict(games_dict).T.rename(columns=['black', 'white', 'winner'])
